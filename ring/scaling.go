...
// Output poly level must be equal or one less than input level.
func (r Ring) DivRoundByLastModulusAux(p0, p1 Poly, levelP int) {

	level := r.level

	for i, s := range r.SubRings[:level] {
		// s.SubThenMulScalarMontgomeryTwoModulus(
		// 	p0.Coeffs[level],
		// 	p0.Coeffs[i],
		// 	r.RescaleConstants[level-1][i],
		// 	p1.Coeffs[i])
		s.SubThenMulScalarMontgomeryTwoModulus(p0.Coeffs[level], p0.Coeffs[i], r.RescaleConstants[level-1][i], p1.Coeffs[i])
	}
}

func (r Ring) DivRoundByLastModulusKernelOpt(p0 Poly, lvl int, moduli, medconsts []uint64, scalarMonts [][]uint64) {
	level := r.level

	for i, s := range r.SubRings[:level+1-lvl] {

		s.SubThenMulScalarMontgomeryTwoModulusAlongMatrix(
			p0.Coeffs[level], p0.Coeffs[level+1-lvl:], scalarMonts[i], moduli, medconsts)
...
func (r Ring) DivRoundByLastModulusManyTimes(p0, p1 Poly, t int) time.Duration {
	now := time.Now()
