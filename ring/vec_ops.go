...
func subthenmulscalarmontgomeryTwoModulusvecAlongMatrix(
	p1 []uint64,
	p2 [][]uint64,
	scalarMonts []uint64,
	// p3 []uint64,
	moduli []uint64,
	mredconstants []uint64) {

	N := len(p1)
	k := len(moduli)
	for i, scalarMont := range scalarMonts {
		modulus := moduli[k-1-i]
		twomodulus := modulus << 1
		mredconstant := mredconstants[i]
		...
		for j := 0; j < N; j = j + 8 {
			...
			x := (*[8]uint64)(unsafe.Pointer(&p1[j]))
			/* #nosec G103 -- behavior and consequences well understood, possible buffer overflow if len(p2)%8 */
			y := (*[8]uint64)(unsafe.Pointer(&p2[i][j]))
			/* #nosec G103 -- behavior and consequences well understood, possible buffer overflow if len(p3)%8 */
